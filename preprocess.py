...
def parse_line_new(line):
    line = line.strip().split()
    e1, e2, relation, hc, conf, rl = line[0].strip(), line[1].strip(), line[2].strip(), float(line[3].strip()), float(line[4].strip()), int(line[5].strip())
    return e1, e2, relation, hc, conf, rl
...
def load_data(filename, entity2id, relation2id, is_unweigted=False, directed=True):
    ...
    for line in lines:
        ...
        triples_data.append(
            (entity2id[e1], relation2id[relation], entity2id[e2]))  # scd: entity 2 id and relation2id
        if not directed:
    ...
    return triples_data, (rows, cols, data), list(unique_entities)  # scd: unique_entities entity list

# for new_triple.txt , It's covered in the paper ,  [[h_id,r_id,t_id,head_coverage, confidence, rule_length],...]
def load_new_data(filename, entity2id, relation2id, is_unweigted=False, directed=True):
    with open(filename) as f:
        lines = f.readlines()

    # this is list for relation triples
    triples_data = []#[[h_id,r_id,t_id,head_coverage, confidence, rule_length],...]

    rows, cols, data, hc, conf, rl = [], [], [], [], [], []#rows=[tails] cols=[heads] data=[rel_id], hc = [head_coverage], conf = [confidence], rl = [rule_length]
    unique_entities = set()#set(entity)
    for line in lines:
        e1, e2, relation, head_coverage, confidence, rule_length = parse_line_new(line)
        unique_entities.add(e1)
        unique_entities.add(e2)
        triples_data.append(
            (entity2id[e1], relation2id[relation], entity2id[e2], head_coverage, confidence, rule_length))
        if not directed:
                # Connecting source and tail entity
            rows.append(entity2id[e1])
            cols.append(entity2id[e2])
            hc.append(head_coverage)
            conf.append(confidence)
            rl.append(rule_length)
            if is_unweigted:
                data.append(1)
            else:
                data.append(relation2id[relation])
    ...
    print("number of unique_entities ->", len(unique_entities))
    return triples_data, (rows, cols, data, hc, conf, rl), list(unique_entities)

def build_data(path='./data/WN18RR/', is_unweigted=False, directed=True):
    entity2id = read_entity_from_id(path + 'entity2id.txt') #dict[str, int]
    relation2id = read_relation_from_id(path + 'relation2id.txt')   #dict[str, int]
    ...
    train_new_triples, train_new_adjacency_mat, unique_new_entities_train = load_new_data(os.path.join(
        path, 'new_triple.txt'), entity2id, relation2id, is_unweigted, directed)
    #[[h,r,t,hc,conf,rl], ...], ([tails],[heads],[relations],[hc],[conf],[rl])
...
#  for selecting random entities
#   such as headTail 781.7747455952955
    headTailSelector = {}
    for i in range(len(relation2id)):
    ...
    return (train_triples, train_adjacency_mat), (validation_triples, valid_adjacency_mat), (test_triples, test_adjacency_mat), \
        (train_new_triples, train_new_adjacency_mat), entity2id, relation2id, headTailSelector, unique_entities_train
