...
def parse_args():
    ...
    args.add_argument("-sa_dim", "--self_attention_dimension", type=int, nargs='+',
                      default=[100, 100], help="dimention of key and value in self-attention")
    args.add_argument("-h_sa", "--nheads_self_attention", type=int, nargs='+',
                      default=[4, 4], help="Multihead attention self-attention")
...
def load_data(args):
    train_data, validation_data, test_data, train_new_data, entity2id, relation2id, headTailSelector, unique_entities_train = build_data(
        args.data, is_unweigted=False,
        directed=True)  # ([[h,r,t,hc,conf,rl], ...],([tails],[heads],[relations],[hc],[conf],[rl]))
    ...
    corpus = Corpus(args, train_data, validation_data, test_data, train_new_data, entity2id, relation2id,
                    headTailSelector,
...
def get_score(train_indices, entity_embed, relation_embed, train_adj_matrix):  # [[h,r,t,hc,conf,l],...]
    shape_adj = train_adj_matrix[2].size()
    head_embeds = entity_embed[[x[0] for x in train_indices]]
    relation_embeds = relation_embed[[x[1] for x in train_indices]]
    tail_embeds = entity_embed[[x[2] for x in train_indices]]
    with torch.no_grad():
        x = 1 - torch.tanh(torch.norm((head_embeds + relation_embeds - tail_embeds), p=1, dim=1))  # N
    train_adj_matrix[2][-1] = x
    return train_adj_matrix

#   train_new_adj_matrix  = (new_adj_indices, new_adj_values, new_other)
#   get_score can calculate the transE
print(Corpus_.train_new_adj_matrix[2])
Corpus_.train_new_adj_matrix = get_score(Corpus_.new_train_triples, entity_embeddings, relation_embeddings,
                                         Corpus_.train_new_adj_matrix)
print(Corpus_.train_new_adj_matrix[2])
...
def train_gat(args):
    ...
    model_gat = SpKBGATModified(entity_embeddings, relation_embeddings, args.entity_out_dim,
                                args.self_attention_dimension, args.entity_out_dim,
                                args.drop_GAT, args.alpha, args.nheads_GAT, args.nheads_self_attention)
    ...
    for epoch in range(args.epochs_gat):
        ...
        for iters in range(num_iters_per_epoch):
            ...
            entity_embed, relation_embed = model_gat(
                Corpus_, Corpus_.train_adj_matrix, Corpus_.train_new_adj_matrix, train_indices,
        ...
        Corpus_.train_new_adj_matrix = get_score(Corpus_.new_train_triples, entity_embed, relation_embed,
                                                 Corpus_.train_new_adj_matrix)
        scheduler.step()
...
def train_conv(args):
    ...
    for epoch in range(args.epochs_conv):
        ...
        scheduler.step()
        print(model_conv.final_entity_embeddings)
        print("Epoch {} , average loss {} , epoch_time {}".format(
            epoch, sum(epoch_loss) / len(epoch_loss), time.time() - start_time))
        epoch_losses.append(sum(epoch_loss) / len(epoch_loss))
