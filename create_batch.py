...
class Corpus:
    def __init__(self, args, train_data, validation_data, test_data, train_new_data, entity2id,
                 relation2id, headTailSelector, batch_size, valid_to_invalid_samples_ratio, unique_entities_train,
                 get_2hop=False):
        self.train_triples = train_data[0]  # [[h,r,t],...]
        # Converting to sparse tensor
        adj_indices = torch.LongTensor(
            [train_data[1][0], train_data[1][1]])  # rows and columns[[tails],[heads]]
        adj_values = torch.LongTensor(train_data[1][2])  # [relations]
        self.train_adj_matrix = (adj_indices, adj_values)  # ([[tails],[heads]],[relations])

        new_adj_indices = torch.LongTensor(
            [train_new_data[1][0], train_new_data[1][1]])  # row, col, head_cov. conf, rule_length
        new_score = np.zeros_like(train_new_data[1][3])
        new_other = torch.FloatTensor(  # scd:  new_score is TransE power func
            [train_new_data[1][3], train_new_data[1][4], train_new_data[1][5] / np.max(train_new_data[1][5]),
             new_score])
        new_adj_values = torch.LongTensor(train_new_data[1][2])
        self.train_new_adj_matrix = (new_adj_indices, new_adj_values, new_other)
        # adjacency matrix is needed for train_data only, as GAT is trained for
        # training data
        self.validation_triples = validation_data[0]  # (validation_triples, valid_adjacency_mat)
        self.test_triples = test_data[0]
        self.new_train_triples = [x[:3] for x in train_new_data[0]]  # [[h,r,t],...]
        ...
        if (get_2hop):
            ...
            self.node_neighbors_2hop = self.get_further_neighbors()  # try to add new triples
        ...
        # self.new_train_indices = np.array(
        #    list(self.new_train_triples)).astype(np.int32)

        # These are valid triples, hence all have value 1
        self.train_values = np.array(
            [[1]] * len(self.train_triples)).astype(np.float32)
        # self.new_train_values = np.array(
        #    [[1]] * len(self.new_train_triples)).astype(np.float32)
        ...
        self.valid_triples_dict = {j: i for i, j in enumerate(
            self.train_triples + self.validation_triples + self.test_triples + self.new_train_triples)}
    ...
    def get_iteration_batch(self, iter_num):
        if (iter_num + 1) * self.batch_size <= len(self.train_indices):
            ...
            if self.invalid_valid_ratio > 0:
                ...
                # of the indices
                self.batch_indices[last_index:(last_index * (self.invalid_valid_ratio + 1)), :] = np.tile(self.batch_indices[:last_index, :], (self.invalid_valid_ratio, 1))
                self.batch_values[last_index:(last_index * (self.invalid_valid_ratio + 1)), :] = np.tile(self.batch_values[:last_index, :], (self.invalid_valid_ratio, 1))
